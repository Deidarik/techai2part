...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_hat, x_centered, std, gamma, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x_hat, x_centered, std, gamma, eps = cache
    N = dout.shape[0]

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_hat, axis=0)
    dx_hat = dout * gamma
    dx = (1.0 / N) / std * (N * dx_hat - np.sum(dx_hat, axis=0) - x_hat * np.sum(dx_hat * x_hat, axis=0))
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    out[n, c, i, j] = np.max(x[n, c, h_s:h_s+HH, w_s:w_s+WW])
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    h_s, w_s = i * stride, j * stride
                    window = x[n, c, h_s:h_s+HH, w_s:w_s+WW]
                    max_val = np.max(window)
                    dx[n, c, h_s:h_s+HH, w_s:w_s+WW] += (window == max_val) * dout[n, c, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_flat, cache = batchnorm_forward(x_transposed, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_flat, dgamma, dbeta = batchnorm_backward(dout_transposed, cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (x_reshaped, x_hat, mean, var, G, eps, gamma, beta)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_reshaped, x_hat, mean, var, G, eps, gamma, beta = cache
    N, C, H, W = dout.shape
    ...
    dx_reshaped = (1.0 / N_group) / std * (N_group * dx_hat - np.sum(dx_hat, axis=(2, 3, 4), keepdims=True) - 
                  ((x_reshaped - mean) / std) * np.sum(dx_hat * (x_reshaped - mean) / std, axis=(2, 3, 4), keepdims=True))
